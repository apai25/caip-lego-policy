...
"""Evaluate sort policy with random cube positions on bimanual robot."""
import hydra
import omegaconf
import redis
import time
...
def run_robot(cfg: omegaconf.DictConfig):
    ...
    with DualUR3ERealEnv(window=False, show_cams=True, save_dir=cfg.save_cam_dir) as env:
        control_freq = 15 
        ...
        print('Evaluating cube sort on random positions. Press Ctrl+Q in the middle of policy execution to exit early.')
        ...
        for i in range(12):
            ...
            env.add_cube_marker(color='y', task='sort')
            env.add_cube_marker(color='b', task='sort')

            # activate recording of real env
            env.deadman_pedal = True
            ...
            for step in range(num_steps):
                ...
                if listener.ctrlq_pressed:
                    ...
                    env.deadman_pedal = False
                    env.save_traj(result=True)
                    break
