...
GRID_RIGHT_QPOS = [
    [-1.8276403586017054, -1.4510453504375, -1.5231715440750122, -2.486485620538229, 0.510415256023407, 0.7963149547576904], 
    [-1.9588235060321253, -1.9284621677794398, -1.1332306861877441, -2.417659421960348, 0.355668306350708, 0.760219931602478], 
...
def run_robot(cfg: omegaconf.DictConfig):
    ...
    # Load a checkpoint
    state_dict = torch.load(cfg.test.tokenizer_weights, map_location="cpu")["model_state"]
    mks, uks = tokenizer.load_state_dict(state_dict, strict=False)
    print("loaded checkpoint from: {}".format(cfg.test.tokenizer_weights))
    ...
    with DualUR3ERealEnv(window=False, show_cams=True) as env:
        ...
        print(f'Evaluating cube pick on 3x4 grid starting at 0. Press Ctrl+Q in the middle of policy execution to exit early.')
        ...
        for i in range(12):
            ...
            for step in range(num_steps):
                ...
                tokenized_actions, _, _ = tokenizer.encode(torch.Tensor(actions[None] - current_state[None, None]).cuda())
                tokenized_actions = tokenizer.decode(tokenized_actions)
                tokenized_actions = (tokenized_actions.detach().cpu().numpy() + current_state[None, None]).squeeze(0)

                print(((tokenized_actions - actions)**2).mean())

                # all_actions[step, step:step+cfg.actor.num_exec] = actions[:cfg.actor.num_exec]
                all_actions[step, step:step+cfg.actor.num_exec] = tokenized_actions[:cfg.actor.num_exec]
