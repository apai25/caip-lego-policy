...
void printTransform(ManusTransform& t) {
	std::cout << "\"pos\": [" << t.position.x << ", " << t.position.y << ", " << t.position.z << "], ";
	std::cout << "\"rot\": [" << t.rotation.w << ", " << t.rotation.x << ", " << t.rotation.y << ", " << t.rotation.z << "], " ;
...
void SDKMinimalClient::Run()
{
	...
	LoadTestSkeleton(0, Side::Side_Left, std::string("LeftHand"));
	LoadTestSkeleton(1, Side::Side_Right, std::string("RightHand"));
	...
	while (m_Running)
	{
		...
		if (m_Skeleton != nullptr && m_Skeleton->skeletons.size() != 0)
		{
			...
			oss << "{" << std::endl;
			for(int i=0; i<m_Skeleton->skeletons.size(); i++) {
				oss << "\"" << names[i] << "\":" << std::endl;
				oss << "[" << std::endl;
				for(int j=0; j<m_Skeleton->skeletons[i].info.nodesCount; j++) {
					SkeletonNode node = m_Skeleton->skeletons[i].nodes[j];
					oss << "{" ;
					oss << "\"id\":" << node.id << ", ";

					ManusTransform t = node.transform;
					oss << "\"pos\": [" << t.position.x << ", " << t.position.y << ", " << t.position.z << "], ";
					oss << "\"rot\": [" << t.rotation.w << ", " << t.rotation.x << ", " << t.rotation.y << ", " << t.rotation.z << "], " ;
					oss << "\"scale\": [" << t.scale.x << ", " << t.scale.y << ", " << t.scale.z << "]"; 

					oss << "}";
					if(j+1 != m_Skeleton->skeletons[i].info.nodesCount) oss << ",";
					oss << std::endl;
				}
				oss << "]";
				if(i+1 != m_Skeleton->skeletons.size()) oss << ",";
				oss << std::endl;
			}
			oss << "}" << std::endl;
			...
			zmq::message_t zmq_message(data.size());
			memcpy(zmq_message.data(), data.data(), data.size());
...
void SDKMinimalClient::LoadTestSkeleton(uint32_t sklId, Side side, std::string name)
{
	uint32_t t_SklIndex = sklId;
	...
	CopyString(t_SKL.name, sizeof(t_SKL.name), name);
	...
	if (!SetupHandNodes(t_SklIndex)) {
		std::cout << "failed setup hand" << std::endl;
		return;
	}
	if (!SetupHandChains(t_SklIndex, side)) {
		std::cout << "failed setup chain" << std::endl;
		return;
	}
	...
	if (t_Res != SDKReturnCode::SDKReturnCode_Success)
	{
		std::cout << "failed load skel" << std::endl;
		return;
	}
}
...
bool SDKMinimalClient::SetupHandChains(uint32_t p_SklIndex, Side side)
{
	...
	const ChainType t_FingerTypes[5] = { 
		ChainType::ChainType_FingerThumb,
