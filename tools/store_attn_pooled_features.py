...
@torch.no_grad()
def store_mae_features(data_root, save_root, demo_name, model_name, model_dir, prompt_text, prompt_key, save_all_features,
                       start, end, fp16, keys):

    # Load the model
    model, _, preprocess = create_model_and_transforms(model_name, pretrained=model_dir)
    ...
    for traj_dir in tqdm(sorted(os.listdir(src_root))[start:end]):
        ...
        if prompt_text is not None:
            prompt = tokenizer([prompt_text]).cuda()
        elif prompt_key in metadata:
            prompt = metadata[prompt_key]
            prompt = tokenizer([prompt]).cuda()
        else:
        ...
        for obs_fname in sorted(os.listdir(src_traj_path)):
            ...
            for cam in keys:
                ...
                if save_all_features:
                    im_emb = model.encode_image(im)
                else:
                    im_emb = model.encode_image_with_text(im, prompt)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model-dir", dest="model_dir", default="/home/bfshi/projects/open_clip/dantong_checkpoints/2024-07-06/vitb16_cc12m_dinov2_16epochs_freezeall_lr5e-4_alpha1/checkpoints/epoch_2.pt")
    ...
    store_mae_features(
        ...
        args.model_name, args.model_dir, args.prompt, args.prompt_key, args.save_all_features,
