...
class SAM_ImageEncoderViT(nn.Module):
    def __init__(
        self,
        img_size: int = 1024,
        patch_size: int = 16,
    ...
    @staticmethod
    def resize_pos_embed(pos_embed, input_shape, mode):
        ...
        """
        assert pos_embed.ndim == 4, 'shape of pos_embed must be [B, H, W, C]'
        # keep dim for easy deployment
        pos_embed = pos_embed.permute(0, 3, 1, 2)
        pos_embed = F.interpolate(
            pos_embed, size=input_shape, align_corners=False, mode=mode)
    ...
    def _pos_embed(self, x, hw_shape):
        ...
        if x.shape[1] != pos_embed.shape[1] or x.shape[2] != pos_embed.shape[2]:
            pos_embed = self.resize_pos_embed(pos_embed, hw_shape, 'bicubic')

        x = x + pos_embed
        return x

    def forward(self, x: torch.Tensor, mode='mean'):
        ...
        if mode == 'mean':
            return x.mean(dim=(-1, -2)).detach().float()
        elif mode == 'all':
            return rearrange(x, 'b c h w -> b (h w) c').detach().float()
        else:
...
def sam_l16(pretrained, **kwargs):
    scratch = pretrained.endswith("none")
    assert scratch or os.path.exists(pretrained)
    model = SAM_ImageEncoderViT(
        depth=24, embed_dim=1024, img_size=1024, mlp_ratio=4, num_heads=16, patch_size=16,
        qkv_bias=True, use_rel_pos=True, global_attn_indexes=[5, 11, 17, 23], window_size=14, out_chans=256, **kwargs)
    # load from checkpoint
    if not scratch:
        load_checkpoint(pretrained, model)
        print("Loaded encoder from: {}".format(pretrained))
...
def sam_h16(pretrained, **kwargs):
    scratch = pretrained.endswith("none")
    assert scratch or os.path.exists(pretrained)
    model = SAM_ImageEncoderViT(
        depth=32, embed_dim=1280, img_size=1024, mlp_ratio=4, num_heads=16, patch_size=16,
        qkv_bias=True, use_rel_pos=True, global_attn_indexes=[7, 15, 23, 31], window_size=14, out_chans=256, **kwargs)
    # load from checkpoint
    if not scratch:
        load_checkpoint(pretrained, model)
        print("Loaded encoder from: {}".format(pretrained))
